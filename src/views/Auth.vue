<template>
    <div class="bg-pattern h-screen flex justify-center items-center">
        <div class="p-5 bg-blumine-600 w-96">
            ...
            <!-- error alert -->
            <div role="alert" class="alert-danger" v-if="is_success(false)" v-html="message.content"></div>
            <form @submit.prevent="connect">
                ...
                <div class="flex flex-col my-5">
                    ...
                    <div class="relative">
                        ...
                        <!-- eye can see icon -->
                        <svg v-if="visible.password" @click="toggle_hide('password')"  class="input-svg" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                        </svg>
                        <!-- eye cannot see icon -->
                        <svg v-else @click="toggle_hide('password')" class="input-svg" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
export default {
    ...
    computed: {
        type() {
            return this.visible.password ? 'text': 'password';
        }
    },
    methods: {
        async connect() {
            const response = await connect(this.email, this.password);
            if (response !== true) this.errors = response;
            else this.$router.push('/hometowns');
        }
    }
}
</script>
